"""LOL.PS scraper using Playwright-rendered HTML."""
...
URL = "https://lol.ps/statistics?lang=ko"
...
def scrape_lolps() -> list[dict[str, Any]]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt == MAX_RETRIES:
                raise RuntimeError("LOL.PS timed out after retries") from exc
            sleep_polite(base=attempt)

    assert html is not None and text is not None

    write_text("debug/raw_html/lolps.html", html)
    write_text("debug/raw_text/lolps.txt", text)
    ...
    # Prefer link-based extraction if champion links exist.
    for a in soup.select('a[href*="champion" i], a[href*="/champs" i], a[href*="/champions" i]'):
        ...
        for _ in range(8):
            ...
            if row.name in {"tr", "li", "div", "article"} and "%" in row_text:
    ...
    if not candidates:
        for line in text.splitlines():
            ...
            if re.search(r"\d+(?:\.\d+)?%", line):
                ...
                if len(pieces) >= 2:
                    champion = pieces[1] if pieces[0].isdigit() else pieces[0]
                    candidates.append((champion, line))

    if not candidates:
        raise RuntimeError("LOL.PS selector broke: no candidate rows found")
